...
import TransactionManager from '../../../main/TransactionManager';
...
class EventsService {
  ...
  static openBrowserView = () => window.electron.ipcRenderer.send(ipcConsts.OPEN_BROWSER_VIEW);

  static updateBrowserViewTheme = ({ isDarkMode }: { isDarkMode: boolean }) =>
    window.electron.ipcRenderer.send(ipcConsts.SEND_THEME_COLOR, { isDarkMode });
  ...
  static listPublicServices = (
    ...
    window.electron.ipcRenderer.invoke(ipcConsts.LIST_PUBLIC_SERVICES, genesisID);
  ...
  static closeWallet = () => window.electron.ipcRenderer.send(ipcConsts.W_M_CLOSE_WALLET);
  ...
  static wipeOut = () => window.electron.ipcRenderer.send(ipcConsts.W_M_WIPE_OUT);
  ...
  static checkFreeSpace = ({ dataDir }: { dataDir: string }) =>
    window.electron.ipcRenderer.invoke(ipcConsts.SMESHER_CHECK_FREE_SPACE, { dataDir });
  ...
  static startSmeshing = async (postSetupOpts: PostSetupOpts) => {
    window.electron.ipcRenderer.send(ipcConsts.SMESHER_START_SMESHING, postSetupOpts);
  };

  static stopSmeshing = ({ deleteFiles }: { deleteFiles: boolean }) =>
    window.electron.ipcRenderer.invoke(ipcConsts.SMESHER_STOP_SMESHING, { deleteFiles });
  ...
  static sendTx = ({
    ...
    window.electron.ipcRenderer.invoke(ipcConsts.W_M_SEND_TX, { fullTx, accountIndex });

  static spawnTx = (
    fee: number,
    accountIndex: number
  ): Promise<ReturnType<TransactionManager['publishSelfSpawn']>> =>
    window.electron.ipcRenderer.invoke(ipcConsts.W_M_SPAWN_TX, { fee, accountIndex });
  ...
  static toggleAutoStart = () => window.electron.ipcRenderer.send(ipcConsts.TOGGLE_AUTO_START);

  /** **************************************   MISC   ***************************************** */

  static reloadApp = () => window.electron.ipcRenderer.send(ipcConsts.RELOAD_APP);
  ...
  static signMessage = ({
    ...
    window.electron.ipcRenderer.invoke(ipcConsts.W_M_SIGN_MESSAGE, { message, accountIndex });
  ...
  static switchApiProvider = (
    ...
    genesisID: string
  ) => window.electron.ipcRenderer.invoke(ipcConsts.SWITCH_API_PROVIDER, { apiUrl, genesisID });
  ...
  static restartNode = () => window.electron.ipcRenderer.send(ipcConsts.N_M_RESTART_NODE);
  ...
  /** **************************************  AUTO UPDATER  **************************************** */
  static downloadUpdate = () => window.electron.ipcRenderer.send(ipcConsts.AU_REQUEST_DOWNLOAD);

  static installUpdate = () => window.electron.ipcRenderer.send(ipcConsts.AU_REQUEST_INSTALL);
}

window.electron.ipcRenderer.on(ipcConsts.N_M_SET_NODE_STATUS, (_event, status: NodeStatus) => {
  store.dispatch(setNodeStatus(status));
});
window.electron.ipcRenderer.on(ipcConsts.N_M_SET_NODE_ERROR, (_event, error: NodeError) => {
...
window.electron.ipcRenderer.on(ipcConsts.T_M_UPDATE_ACCOUNT, (_event, request) => {
  store.dispatch(
    updateAccountData({
...
window.electron.ipcRenderer.on(ipcConsts.T_M_UPDATE_REWARDS, (_event, request) => {
...
window.electron.ipcRenderer.on(ipcConsts.SMESHER_SEND_SMESHING_CONFIG, (_event, request) => {
...
window.electron.ipcRenderer.on(ipcConsts.AU_AVAILABLE, (_, info: UpdateInfo) => {
  store.dispatch(updaterSlice.actions.updateAvailable(info));
});

window.electron.ipcRenderer.on(ipcConsts.AU_DOWNLOAD_COMPLETE, (_, info: UpdateInfo) => {
...
window.electron.ipcRenderer.on(ipcConsts.AU_DOWNLOAD_PROGRESS, (_, info: ProgressInfo) => {
