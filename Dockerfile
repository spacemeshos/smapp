...
FROM ubuntu:18.04 
RUN apt update && DEBIAN_FRONTEND=noninteractive apt install -y xpra libnss3 libgtk-3-0 libxss1 libasound2 desktop-file-utils x11-apps

RUN adduser --disabled-password --gecos '' spacemesh 
RUN chown -R spacemesh /home/spacemesh

COPY --from=builder /home/spacemesh/release/smapp_*_amd64.deb /tmp
RUN dpkg -i /tmp/smapp_*_amd64.deb ; apt install -y -f && chmod 755 /opt/Spacemesh/node/go-spacemesh
